...
pub static mut MADGWICK: Option<&'static Mutex<NoopRawMutex, Madgwick>> = None;
pub static mut COMPLEMENTARY: Option<&'static Mutex<NoopRawMutex, ComplementaryFilter>> = None;
...
pub async fn read_imu_task(mut imu: ImuPack<'static, I2c<'static, I2C0, Async>>) {
    // Initialization
    // let madgwick = unsafe { MADGWICK.unwrap() };
    let complementary = unsafe { COMPLEMENTARY.unwrap() };
    ...
    loop {
        match imu.read_all().await {
            Ok((accel, gyro, mag)) => {
                ...
                */
                let mut lock = complementary.lock().await;
                lock.update(gyro, accel, mag, 0.01);
